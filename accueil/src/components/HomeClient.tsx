...
interface PersonalData {
  personal: { handle: string };
  about: { role: string };
}
interface ExperienceData { experience: ExperienceItem[]; }
        ...
        <div className="w-full">
          <div className="flex items-start justify-between mb-12">
            <div>
              ...
              <AnimatedSection animation="fade-in-up" delay={400}>
                <p className="text-gray-500 mb-6">{personalData.about?.role}</p>
              </AnimatedSection>
              <AnimatedSection animation="slide-in-left" delay={600}>
                <SocialIcons shortcuts={homeData.homeShortcuts ?? []} />
              </AnimatedSection>
            </div>
          ...
          <div className="space-y-6">
            {(experienceData.experience ?? []).map((item, index) => (
              <AnimatedSection key={index} animation="fade-in-up" delay={1000 + index * 200}>
                <TimelineItem
                  title={item.title}
