...
interface PersonalData {
  personal: { name: string; handle: string; email: string; location: string; website: string; linkedin: string; github: string; socials: Social[]; };
  about: { role: string; description: string; };
}
interface ExperienceData { experience: ExperienceItem[]; education: ExperienceItem[]; }
interface ProjectsData { projects: Project[]; }
interface SkillsData { skills: Skill[]; }
interface CVData { cv: { title: string; summary: string; skills: Record<string, string>; languages: Record<string, string>; }; }
        ...
        <AnimatedSection animation="fade-in-up" className="mb-16">
          ...
          <div className="flex gap-4">
            ...
            <PDFDownloadButton
              ...
              cv={cvData?.cv ?? { title: '', summary: '', skills: {}, languages: {} }}
            />
          </div>
        </AnimatedSection>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-3">
            {(skillsData.skills ?? []).map((skill, index) => (
              <AnimatedSection key={index} animation="fade-in" delay={100 + index * 50}>
